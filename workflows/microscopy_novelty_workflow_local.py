...
def select_claims_interactive(claims):
    ...
    for i, claim in enumerate(claims):
        print(f"\n[{i+1}] {claim.get('has_anyone', 'No question formulated')}")
...
class MicroscopyNoveltyAssessmentWorkflowLocal:
    """
    Workflow for analyzing experimental results and assessing their novelty.
    Based on exp2lit_local.py structure.
    """
    
    def __init__(self, futurehouse_api_key: str,
        ...
        self.analysis_agent = AnalysisAgent(model_name='gemma3_12B_local', fft_nmf_settings=fft_nmf_settings)
    ...
    def run_complete_workflow(self, image_path: str, system_info: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, claim in enumerate(claims):
                ...
                print(f"   Has Anyone Question: {claim.get('has_anyone', 'No question formulated')}")
        ...
        try:
            ...
            for i, claim in enumerate(selected_claims):
                has_anyone_question = claim.get("has_anyone")
